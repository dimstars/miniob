...
class BloomFilter {
 ...
 public:
   ...
   */
  inline BloomFilter(const ValueType **p_data_p, const ValueEqualityChecker &p_value_eq_obj = ValueEqualityChecker{},
                     const ValueHashFunc &p_value_hash_obj = ValueHashFunc{})
      : data_p{p_data_p}, value_count{0}, value_eq_obj{p_value_eq_obj}, value_hash_obj{p_value_hash_obj} {
#ifdef BLOOM_FILTER_ENABLED
  ...
  inline bool __ExistsScalar(const ValueType &value) {
    ...
    if ((bit_array_0[(hash_value & BYTE_OFFSET_MASK) >> 3] & (0x1 << (hash_value & BIT_OFFSET_MASK))) == 0x00) {
      return false;
    }
#endif
