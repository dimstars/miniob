...
class SortedSmallSet {
 ...
 public:
  SortedSmallSet(ValueType *p_data, const ValueComparator &p_value_cmp_obj = ValueComparator{},
      ...
      : data{p_data}, start_p{p_data}, end_p{p_data}, value_cmp_obj{p_value_cmp_obj}, value_eq_obj{p_value_eq_obj} {}
   ...
   */
  inline ValueType *GetBegin() { return start_p; }

  inline ValueType *GetEnd() { return end_p; }
   ...
   */
  inline bool IsEmpty() { return start_p == end_p; }
   ...
   */
  inline ValueType &GetFront() { return *start_p; }

  /*
   * GetSize() - Returns the size of the container, in element count
   */
  inline std::ptrdiff_t GetSize() const { return end_p - start_p; }
