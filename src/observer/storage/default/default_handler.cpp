...
RC DefaultHandler::create_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, const char * const *attribute_names, int attribute_num) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_names, attribute_num);
}

RC DefaultHandler::create_unique_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, const char * const *attribute_names, int attribute_num) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_names, attribute_num, true);
}
